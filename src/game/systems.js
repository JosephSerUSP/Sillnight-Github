...
import { Scene_Battle } from './scenes.js';
...
export const Systems = {
    ...
    Events: {
        ...
        show(title, content) {
            shellUI.eventModal.showEvent(title, content);
        },
        close() {
            shellUI.closeEvent();
        },
        ...
        recruit() {
            ...
            offers.forEach(def => {
                const row = document.createElement('div');
                row.className = 'flex justify-between items-center bg-gray-900 p-2 border border-gray-700';
                row.innerHTML = `<div class="flex items-center gap-2">${shellUI.spriteMarkup(def, 'h-10 w-10 object-contain')}<div><div class="text-yellow-100">${def.name}</div><div class="text-xs text-gray-500">HP ${def.baseHp}</div></div></div>`;
                const btn = document.createElement('button');
                btn.className = 'text-xs border border-gray-600 px-2 py-1 hover:bg-white hover:text-black';
    ...
    Battle: {
        ...
        startEncounter() {
            ...
            setTimeout(() => {
                ...
                sceneManager.changeScene(new Scene_Battle(Systems, shellUI));
        ...
        resumeAuto() {
            ...
            shellUI.togglePlayerTurn(false);
            GameState.battle.playerTurnRequested = false;
            const btn = document.getElementById('btn-player-turn');
        ...
        processNextTurn() {
            ...
            shellUI.showBanner(`${unit.name} used ${action.name}!`);
