    ...
    Events: {
        ...
        shop() {
            ...
            stock.forEach(s => {
                ...
                row.innerHTML = `<div><div class="sn-title">${name}</div><div class="sn-hint">${desc}</div><div class="sn-text-dim">${price} G</div></div>`;
        ...
        recruit() {
            ...
            offers.forEach(def => {
                ...
                sprite.innerHTML = Systems.UI.spriteMarkup(def, 'h-10 w-10 object-contain');
                const info = document.createElement('div');
                info.innerHTML = `<div class="sn-title">${def.name}</div><div class="sn-hint">HP ${def.baseHp}</div>`;
                const infoWrap = document.createElement('div');
                infoWrap.className = 'sn-row';
                infoWrap.appendChild(sprite);
        ...
        shrine() {
            ...
            msg.className = 'sn-column';
            msg.innerHTML = `<div class="sn-title">Your party feels rejuvenated.</div><button class="sn-button">Continue</button>`;
        ...
        trap() {
            ...
            const msg = document.createElement('div');
            msg.className = 'sn-column';
            msg.innerHTML = `<div class="sn-text-danger">A trap harms your party!</div><button class="sn-button">Continue</button>`;
            msg.querySelector('button').onclick = () => { Systems.Events.close(); };
            this.show('TRAP', msg);
        }
    },
        ...
        processNextTurn() {
            ...
            if (win) {
                ...
                Systems.UI.showModal(`
                    <div class="sn-title">VICTORY</div>
                    <div>Found ${gold} Gold</div>
                    <div>Party +${xp} XP</div>
                    <button class="sn-button sn-button--primary" onclick="Game.Views.UI.closeModal(); Game.Views.UI.switchScene(false);">CONTINUE</button>
                `);
            } else {
                GameState.ui.mode = 'EXPLORE';
                Systems.Battle3D.setFocus('neutral');
                Systems.UI.showModal(`
                    <div class="sn-text-danger sn-title">DEFEATED</div>
                    <button class="sn-button" onclick="location.reload()">RESTART</button>
                `);
            }
        }
    },
...
const uiManager = new UIManager({ state: GameState, data: Data, resolveAssetPath });
Systems.UI = uiManager;
uiManager.bindSystems(Systems);
uiManager.init();
Log.setRenderer((msg, color) => uiManager.addLogEntry(msg, color));
