...
export const Systems = {
    ...
    Battle3D: {
        ...
        setupScene(allies, enemies) {
            this.group.clear();
            this.sprites = {};
            this.effectHandles = this.effectHandles || {};
        ...
        // Dispatches an Effekseer effect (with billboard fallback) at the given sprite.
        playEffect(effectKey, sprite) {
            const def = Data.effects[effectKey] || Data.effects.default || {};
            if (!sprite) return Promise.resolve();
            return new Promise((resolve) => {
                const duration = def.duration || 600;
                const pos = sprite.position;
                if (this.effekseerContext && def.file) {
                    let handle = this.effectHandles[def.id];
                    if (!handle) {
                        const resolved = resolveAssetPath(def.file);
                        if (resolved) {
                            handle = this.effekseerContext.loadEffect(resolved, def.scale || 1, () => {});
                            this.effectHandles[def.id] = handle;
                        }
                    }
                    if (handle) {
                        const instance = this.effekseerContext.play(handle, 1.0);
                        instance.setLocation(pos.x, pos.y, pos.z);
                        instance.setScale(def.scale || 1, def.scale || 1, def.scale || 1);
                        instance.setSpeed(def.speed || 1);
                        setTimeout(resolve, duration);
                        return;
                    }
                }
                const billboard = this.createBillboard(def.icon || '✨', pos.x, pos.y, pos.z, def.scale || 1);
                this.group.add(billboard);
                setTimeout(() => { this.group.remove(billboard); resolve(); }, duration);
            });
        },
        async runActionSequence(action, actor, targets, onImpact) {
            const sequence = Data.actionSequences[action.actionSequence] || Data.actionSequences.default;
            const steps = sequence?.steps || [];
            const actorSprite = this.sprites[actor.uid];
            const targetSprites = targets.map(t => this.sprites[t.uid]).filter(Boolean);
            const hop = (sprite) => new Promise((resolve) => {
                ...
                const anim = setInterval(() => {
                    t += 0.2;
                    sprite.position.z = baseZ + Math.sin(t) * 0.6;
                    if (t >= Math.PI) { clearInterval(anim); sprite.position.z = baseZ; resolve(); }
                }, interval);
            });
            for (const step of steps) {
                if (step.type === 'wait') { await new Promise(res => setTimeout(res, step.wait || 0)); continue; }
                if (step.type === 'cast') {
                    await hop(actorSprite);
                    await this.playEffect(step.effect, actorSprite);
                } else if (step.type === 'impact') {
                    const list = step.subject === 'actor' ? [actorSprite] : targetSprites;
                    await Promise.all(list.map(s => this.playEffect(step.effect, s)));
                    if (step.apply && onImpact) onImpact();
                }
            }
        },
    ...
    */
    Battle: {
        ...
        async processNextTurn() {
            ...
            const delay = await this.takeTurn(unit);
            if (delay === null) return;
            setTimeout(() => this.processNextTurn(), delay || 800);
        },
        async takeTurn(unit) {
            ...
            const results = targets.map((t) => {
                if (action.category === 'damage' || action.category === 'heal') {
                    const base = Math.floor((action.power || 0) + (action.scaling || 0) * unit.level);
                    const attackMult = this.elementMultiplier(action.element, unit, 'attacker');
                    const defenseMult = this.elementMultiplier(action.element, t, 'defender');
            ...
            await Systems.Battle3D.runActionSequence(action, unit, targets, applyImpact);

            const alliesDown = GameState.battle.allies.every(u => u.hp <= 0);
            const foesDown = GameState.battle.enemies.every(u => u.hp <= 0);
            if (foesDown) { this.end(true); return null; }
            if (alliesDown) { this.end(false); return null; }
            return (action.actionSequence === 'ritual' ? 1400 : 900);
        },
        end(win) {
