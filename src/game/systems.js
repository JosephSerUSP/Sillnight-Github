...
import { $gameParty, $gameMap, $gameSystem } from './globals.js';
...
export const Systems = {
    ...
    Explore: {
        ...
        move(dx, dy) {
            ...
            if (tile !== 1) {
                $gameMap.playerPos.x = newX;
                $gameMap.playerPos.y = newY;
                this.checkTile(tile);
                this.render();
            }
        },
        ...
        render() {
            ...
            for (let y = 0; y < mapCfg.height; y++) {
                for (let x = 0; x < mapCfg.width; x++) {
                    ...
                    if (dist < mapCfg.viewDistance) $gameMap.visited[y][x] = true;
    ...
    Events: {
            ...
            stock.forEach(s => {
                ...
                btn.onclick = () => {
                    if ($gameParty.gold >= price) {
                        ...
                        if (s.type === 'item') {
                            $gameParty.items[s.id] = ($gameParty.items[s.id] || 0) + 1;
                        } else {
                            $gameParty.equipment[s.id] = ($gameParty.equipment[s.id] || 0) + 1;
                        }
        ...
        recruit() {
            ...
            offers.forEach(def => {
                ...
                btn.onclick = () => {
                    const empty = $gameParty.actors.findIndex(u => u === null);
                    const unit = {
                        ...
                        level: $gameSystem.ui.floor,
                        maxhp: Math.round(def.baseHp * (1 + def.hpGrowth * ($gameSystem.ui.floor - 1))),
                        hp: Math.round(def.baseHp * (1 + def.hpGrowth * ($gameSystem.ui.floor - 1))),
                        exp: 0,
                    ...
                    $gameParty.roster.push(unit);
                    if (empty !== -1) {
                        unit.slotIndex = empty;
                        $gameParty.actors[empty] = unit;
        ...
        trap() {
            ...
            const damage = (u) => {
                const dmg = Math.ceil(u.mhp() * 0.2);
                u.hp = Math.max(0, u.hp - dmg);
                if (u.hp === 0) Log.battle(`${u.name} was knocked out by the trap!`);
            };
            $gameParty.actors.forEach(u => { if (u) damage(u); });
            window.Game.Windows.Party.refresh();
            const msg = document.createElement('div');
            msg.className = 'text-center space-y-2';
    ...
    Battle3D: {
        ...
        animate() {
            ...
            if ($gameSystem.ui.mode === 'BATTLE_WIN') {
                cs.angle += 0.005;
            } else {
...
showDamageNumber(uid, val, isCrit = false) {
    ...
    Triggers: {
        fire(eventName, ...args) {
            const allUnits = [...($gameSystem.battle?.allies || []), ...($gameSystem.battle?.enemies || [])];
                ...
                case 'onUnitDeath':
                    if (trait.type === 'on_death_cast') {
                        ...
                        if (deadUnit.uid === unit.uid) {
                            ...
                            if (skill) {
                                const enemies = $gameSystem.battle.enemies.filter(e => e.hp > 0);
    ...
    Battle: {
        ...
        startEncounter() {
            ...
            setTimeout(() => {
                ...
                const allies = $gameParty.actors.filter(u => u !== null).map(u => u);
                const floor = window.Game.ui.floor;
                ...
                Systems.Battle3D.setupScene($gameSystem.battle.allies, $gameSystem.battle.enemies);
        ...
        nextRound() {
            if (!$gameSystem.battle) return;
            $gameSystem.battle.roundCount++;
            $gameSystem.battle.phase = 'ROUND_START';
            Systems.Battle3D.setFocus('neutral');
            if ($gameSystem.battle.allies.every(u => u.hp <= 0)) return this.end(false);
            if ($gameSystem.battle.enemies.every(u => u.hp <= 0)) return this.end(true);

            [...$gameSystem.battle.allies, ...$gameSystem.battle.enemies].forEach(u => {
                if (u.status) {
                    u.status = u.status.filter(s => s !== 'guarding');
                }
            });

            Log.battle(`--- Round ${$gameSystem.battle.roundCount} ---`);
            if ($gameSystem.battle.playerTurnRequested) {
                $gameSystem.battle.phase = 'PLAYER_INPUT';
                $gameSystem.battle.playerTurnRequested = false;
            ...
            $gameSystem.battle.queue = allUnits;
            $gameSystem.battle.turnIndex = 0;
            this.processNextTurn();
        },
        requestPlayerTurn() {
            if (window.Game.ui.mode === 'BATTLE') {
                $gameSystem.battle.playerTurnRequested = true;
        ...
        swapUnits(idx1, idx2) {
            const u1 = $gameParty.actors[idx1];
            const u2 = $gameParty.actors[idx2];
            $gameParty.actors[idx1] = u2;
            $gameParty.actors[idx2] = u1;
            if ($gameParty.actors[idx1]) $gameParty.actors[idx1].slotIndex = idx1;
            if ($gameParty.actors[idx2]) $gameParty.actors[idx2].slotIndex = idx2;
            window.Game.Windows.Party.refresh();
            if (window.Game.ui.mode === 'BATTLE') {
                $gameSystem.battle.allies = $gameParty.actors.filter(u => u !== null);
                Systems.Battle3D.setupScene($gameSystem.battle.allies, $gameSystem.battle.enemies);
            }
        ...
        processNextTurn() {
            window.Game.Windows.Party.refresh();
            if ($gameSystem.battle.turnIndex >= $gameSystem.battle.queue.length) {
                setTimeout(() => this.nextRound(), 1000);
                return;
            }
            const unit = $gameSystem.battle.queue[$gameSystem.battle.turnIndex++];
            if (unit.hp <= 0) {
                this.processNextTurn();
                return;
            }
            Systems.Triggers.fire('onTurnStart', unit);
            const isAlly = $gameSystem.battle.allies.some(a => a.uid === unit.uid);
            Systems.Battle3D.setFocus(isAlly ? 'ally' : 'enemy');
            const enemies = isAlly ? $gameSystem.battle.enemies : $gameSystem.battle.allies;
            const friends = isAlly ? $gameSystem.battle.allies : $gameSystem.battle.enemies;
            ...
            const applyResults = () => {
                results.forEach(({ target, value, effect }) => {
                    ...
                    switch (effect.type) {
                        case 'hp_damage':
                            ...
                            break;
                        case 'hp_heal':
                            target.hp = Math.min(target.mhp(), target.hp + value);
                            ...
                            break;
                        case 'hp_heal_ratio':
                            const healAmount = Math.floor(target.mhp() * parseFloat(effect.formula));
                ...
                window.Game.Windows.Party.refresh();
                if (window.Game.battle.allies.every(u => u.hp <= 0) || window.Game.battle.enemies.every(u => u.hp <= 0)) {
                    window.Game.battle.turnIndex = 999;
                }
            };
            ...
            if (win) {
                ...
                Systems.Triggers.fire('onBattleEnd', [...window.Game.battle.allies, ...window.Game.battle.enemies].filter(u => u && u.hp > 0));
                Systems.Battle3D.setFocus('victory');
                const gold = window.Game.battle.enemies.length * Data.config.baseGoldPerEnemy * window.Game.ui.floor;
                const baseXp = window.Game.battle.enemies.length * Data.config.baseXpPerEnemy * window.Game.ui.floor;
