...
import { spriteMarkup as uiSpriteMarkup } from './ui/windows.js';
...
export const Systems = {
    ...
    */
    ...
    /*
     * UI system: delegates to the UIManager-based window framework.
     */
    UI: {
        updateHUD() { UI.refreshAll(); },
        renderParty() { UI.refreshParty(); },
        toggleParty() { UI.toggleParty(); },
        toggleInventory() { UI.toggleInventory(); },
        toggleFormationMode() { UI.toggleFormation(); },
        spriteMarkup(unit) { return uiSpriteMarkup(unit); },
        startEquipFlow(id) { UI.startEquipFlow(id); },
        showCreatureModal(unit) { UI.showCreature(unit); },
        showEvent(title, html) {
            const div = document.createElement('div');
            if (typeof html === 'string') div.innerHTML = html; else div.appendChild(html);
            UI.showEvent(title, div);
        },
        switchScene(isBattle) {
            const explore = document.getElementById('explore-layer');
            const battle = document.getElementById('battle-layer');
            if (isBattle) {
                explore.classList.add('hidden-scene');
                explore.classList.remove('active-scene');
                battle.classList.remove('hidden-scene');
                battle.classList.add('active-scene');
            } else {
                explore.classList.remove('hidden-scene');
                explore.classList.add('active-scene');
                battle.classList.add('hidden-scene');
                battle.classList.remove('active-scene');
            }
        },
        showBanner(text) { UI.showBanner(text); },
        togglePlayerTurn() {},
        showModal(html) {
            const div = document.createElement('div');
            div.innerHTML = html;
            UI.showEvent('MESSAGE', div);
        },
        closeModal() { UI.closeModal(); },
        closeEvent() { UI.closeModal(); }
    }
};
