...
export const Systems = {
    ...
    Map: {
        ...
        resolveTile(code) {
            if (code === 2) { 
                $gameMap.map()[$gameMap.playerPos().y][$gameMap.playerPos().x] = 0;
                Systems.Battle.startEncounter();
            } else if (code === 3) { 
                $gameMap.incrementFloor();
                Log.add('Descended...');
    ...
    Events: {
            ...
            stock.forEach(s => {
                ...
                btn.onclick = () => {
                    if ($gameParty.gold() >= price) {
                        ...
                        const item = s.type === 'item' ? Data.items[s.id] : Data.equipment[s.id];
        ...
        recruit() {
            ...
            offers.forEach(def => {
                ...
                btn.onclick = () => {
                    const empty = $gameParty.members().findIndex(u => u === null);
                    const unit = {
                        ...
                        level: $gameMap.floor(),
                        maxhp: Math.round(def.baseHp * (1 + def.hpGrowth * ($gameMap.floor() - 1))),
                        hp: Math.round(def.baseHp * (1 + def.hpGrowth * ($gameMap.floor() - 1))),
                        exp: 0,
                    ...
                    $gameParty._roster.push(unit);
                    if (empty !== -1) {
                        unit.slotIndex = empty;
                        $gameParty._activeSlots[empty] = new Game_Actor(unit);
        ...
        shrine() {
            ...
            $gameParty.members().forEach(member => {
                member.applyHealing(member.getMaxHp());
            });
            UI.renderParty();
        ...
        trap() {
            ...
            $gameParty.members().forEach(member => {
                const dmg = Math.ceil(member.getMaxHp() * 0.2);
                member.changeHp(-dmg, true);
                if (member.hp() === 0) Log.battle(`${member.name} was knocked out by the trap!`);
            });
            UI.renderParty();
            const msg = document.createElement('div');
            msg.className = 'text-center space-y-2';
    ...
    Battle: {
        ...
        startEncounter() {
            ...
            setTimeout(() => {
                ...
                const allies = $gameParty.members();
        ...
        swapUnits(idx1, idx2) {
            ...
            UI.renderParty();
            if ($gameSystem.ui.mode === 'BATTLE') {
                $gameSystem.battle.allies = $gameParty.members();
                Systems.Battle3D.setupScene($gameSystem.battle.allies, $gameSystem.battle.enemies);
            }
        ...
        processNextTurn() {
            ...
            const applyResults = () => {
                ...
                UI.renderParty();
                if ($gameSystem.battle.allies.every(u => u.hp <= 0) || $gameSystem.battle.enemies.every(u => u.hp <= 0)) {
                    $gameSystem.battle.turnIndex = 999;
                }
            };
            ...
            if (win) {
                ...
                $gameSystem.ui.mode = 'BATTLE_WIN';
                Systems.sceneHooks?.onBattleEnd?.();
                Systems.Triggers.fire('onBattleEnd', [...$gameSystem.battle.allies, ...$gameSystem.battle.enemies].filter(u => u && u.hp > 0));
                Systems.Battle3D.setFocus('victory');
                const gold = $gameSystem.battle.enemies.length * Data.config.baseGoldPerEnemy * $gameMap.floor();
                const baseXp = $gameSystem.battle.enemies.length * Data.config.baseXpPerEnemy * $gameMap.floor();
                $gameParty.gainGold(gold);
                let finalXp = baseXp;
                $gameParty.members().forEach(p => {
                    if (p) {
                ...
                $gameParty.members().forEach(p => {
                    if (p) {
                        const heal = Math.floor(p.getMaxHp() * 0.25);
                        p.applyHealing(heal);
                    }
                });
                UI.updateHUD();
