...
export class Game_BattlerBase {
    ...
    set mp(value) {
        ...
        this.refresh();
        if (diff !== 0) {
            Services.events.emit('battler:change', { unit: this, property: 'mp', value: this._mp, diff });
        }
    }
    ...
    set tp(value) {
        ...
        this.refresh();
        if (diff !== 0) {
            Services.events.emit('battler:change', { unit: this, property: 'tp', value: this._tp, diff });
        }
    }
     ...
     * @note Parameters `mpd` (MP Drain), `mxa` (Max Actions), and `mxp` (Max Passives) from `gameDesign.md` are pending implementation.
