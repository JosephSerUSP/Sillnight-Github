...
class ShellUI {
    ...
    updateHUD() {
        document.getElementById('hud-floor').innerText = $gameParty.floor;
            ...
            if (u) {
                ...
                div.onclick = () => {
                    if (Game.battle && Game.battle.phase === 'PLAYER_INPUT' || Game.ui.formationMode) {
                        ...
                        if (selected) {
                            const idx = parseInt(selected.dataset.idx);
                            const u1 = $gameParty.actors[idx];
                            $gameParty.actors[idx] = null;
                            $gameParty.actors[i] = u1;
    ...
    onPartySlotClick(element) {
        ...
        if (selected) {
            ...
            if (fromIsReserved && !toIsReserved && !toUnit && activePartySize >= 6) {
    ...
    startEquipFlow(id) {
        ...
        const list = $gameParty.actors.map(u => ({ owner: u, id: u.equipmentId, source: 'unit' })).filter(x => x.id);
        const inv = Object.keys($gameParty.equipment).map(key => ({ owner: null, id: key, source: 'inventory' }));
        const options = [...list, ...inv];
        const first = this.activeModalUnit || $gameParty.actors.find(Boolean);
    ...
    equipFromInventory(target, equipmentId) {
        ...
        if (previous) {
            $gameParty.equipment[previous] = ($gameParty.equipment[previous] || 0) + 1;
        }
        $gameParty.equipment[equipmentId] = count - 1;
        if ($gameParty.equipment[equipmentId] <= 0) delete $gameParty.equipment[equipmentId];
    ...
    async switchScene(toBattle, onComplete) {
